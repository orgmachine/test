...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["pages-wallet-wallet-module"], {
  ...
  function srcAppPagesWalletWalletPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var src_app_services_razorpay__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(
    ...
    /*#__PURE__*/
    function () {
      function WalletPage(api, util, router, razorpayService, cd) {
        ...
        this.amounts = [100, 200, 500, 1000, 2000, 5000, 10000, 20000];
      ...
      _createClass(WalletPage, [{
        key: "setStateAsActive",
        value: function setStateAsActive(amount) {
        ...
        value: function getWalletBalance() {
          ...
          this.api.getWallet(uid).then(function (data) {
            if (data && data.length != 0) {
              _this.wallet_money = data[0];
              _this.balance = _this.wallet_money.amount;
              _this.createdAt = _this.wallet_money.createdAt;
            }
          }).catch(function (err) {
            if (err) {
              console.log(err);
        ...
        value: function getProfile() {
          ...
          this.api.getMyProfile(uid).then(function (data) {
            ...
            if (data) {
              console.log(data);
              _this2.profile = data; // this.phone = data.phone;
              // this.name = data.name;
              // this.email = data.email;

              _this2.token = data.fcm_token;
        ...
        value: function razo(payment_id) {
          ...
          var uid = sessionStorage.getItem('uid');
          var date = new Date();
          var createdAt = moment__WEBPACK_IMPORTED_MODULE_5__(date).format("YYYY-MM-DD hh:mm:ss A");
          ...
          this.api.addTransaction(id.toString(), param).then(function (data) {
            ...
            _this3.api.addWalletTransaction(id.toString(), param).then(function (userData) {
              var uid = sessionStorage.getItem('uid');
              var date = new Date();
              var updatedAt = moment__WEBPACK_IMPORTED_MODULE_5__(date).format("YYYY-MM-DD hh:mm:ss A");
              var id = Math.floor(100000000 + Math.random() * 900000000);
              var total_amount = _this3.activeAmount + _this3.balance;
              var post = {
                uid: uid,
                createdAt: _this3.createdAt,
                updatedAt: updatedAt,
                amount: total_amount
              };

              _this3.api.addMoneyToWallet(uid, post).then(function (data) {
                _this3.util.hide();

                _this3.util.showToast('Money added to wallet successfully', 'primary', 'bottom');

                _this3.getWalletBalance();

                _this3.util.publishWalletStatus(true);

                _this3.sendWalletNotification(); // this.router.navigate(['/tabs/tab1']);
        ...
        value: function sendWalletNotification() {
          ...
          var msg = 'Thank you for your recent payment. You have successfully added ' + this.activeAmount + ' rupees in your Way2Lab Wallet';
