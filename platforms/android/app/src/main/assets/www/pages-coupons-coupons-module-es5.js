...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["pages-coupons-coupons-module"], {
  ...
  function srcAppPagesCouponsCouponsPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    /*#__PURE__*/
    function () {
      function CouponsPage(api, route, util, navCtrl, menu) {
        ...
        this.labdata = [];
        this.cartdata = [];
        this.labid = ""; //this.menu.enable(false);
      ...
      _createClass(CouponsPage, [{
        ...
        value: function ngOnInit() {
          ...
          this.route.queryParams.subscribe(function (data) {
            _this.labdata = localStorage.getItem('labid');
            _this.cartdata = Object(flatted__WEBPACK_IMPORTED_MODULE_7__["parse"])(localStorage.getItem('cartdata'));
            _this.labid = _this.labdata;
          });
        }
      }, {
        key: "getCoupon",
        value: function getCoupon() {
          ...
          this.api.getUsedCoupon(uid).then(function (data) {
            console.log('list=====>', data);

            if (data) {
              _this2.usedCoupon = data;
            }
        ...
        value: function getOffers() {
          ...
          this.api.getOffers().then(function (data) {
            ...
            if (data && data.length) {
              var currnetDate = moment__WEBPACK_IMPORTED_MODULE_3__().subtract(1, 'days').format('YYYY-MM-DD');
              ...
              data.forEach(function (element) {
                ...
                if (element && element.status == 1 && moment__WEBPACK_IMPORTED_MODULE_3__(element.expire).isAfter(currnetDate)) {
                  ...
                  try {
                    for (var _iterator = element.branch[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                      var x = _step.value;

                      if (x.item_id == _this3.labid) {
                        lab_exist = 1;
                      }
                    }
                  } catch (err) {
          ...
          if (item && item.status == 1) {
            ...
            if (this.cartdata.selling_price >= item.min) {
              if (this.usedCoupon.length != 0) {
                ...
                this.api.getUsedCouponById(uid, item.id).then(function (data) {
                  console.log(data);

                  if (data && data.length) {
                    if (data[0].used_coupon < item.limit) {
                      _this4.util.showToast('Coupon Applied', 'success', 'bottom');

                      _this4.util.publishCoupon(item);

                      _this4.navCtrl.back();
                    } else {
                      console.log('hi');

                      _this4.util.errorToast('Coupon limits exceeded ' + item.limit);
                    }
                  } else {
                    _this4.util.showToast('Coupon Applied', 'success', 'bottom');
